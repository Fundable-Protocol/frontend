...
export async function GET(request: NextRequest) {
  try {
    ...
    if (user_address) {
      ...
      const { distributions } =
        await DistributionService.getDistributionsByUser(user_address, 1, 1000);
      ...
      const totalRecipients = completedDistributions.reduce(
        (sum, dist) => sum + dist.total_recipients,
        0
      );
